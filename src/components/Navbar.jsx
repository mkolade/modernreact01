       ...
       <div className=' sm:flex hidden justify-end items-center flex-1'>
          <ul className='list-none  flex  '>
              {navLinks.map((navLink,index) =>(
                <li 
                  key={navLink.id} 
                  className={`font-poppins font-normal cursor-pointer text-[16px] text-white hover:text-secondary ${index === navLinks.length - 1 ? 'mr-0' : 'mr-12'}`}
                >
                  <a href={`#${navLink.id}`}>
                    {navLink.title}
                  </a>
            ...
            <div className={`${toggle ? 'flex' : 'hidden'} text-white p-6 bg-black-gradient absolute top-20 right-0 min-w-[140px] rounded-xl mx-4 my-2 sidebar`}>
              <ul className='list-none sm:flex flex-col  justify-end items-center flex-1'>
                  {navLinks.map((navLink,index) =>(
                    <li 
                      key={navLink.id} 
                      className={`font-poppins font-normal cursor-pointer text-[16px] text-white hover:text-secondary ${index === navLinks.length - 1 ? 'mb-0' : 'mb-4'}`}
                    >
                      <a href={`#${navLink.id}`}>
                        {navLink.title}
                      </a>
